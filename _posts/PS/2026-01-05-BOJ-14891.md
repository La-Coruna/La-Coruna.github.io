...
## 🔗Link
{:.no-top-margin}
[문제](https://boj.kr/14891)  
[풀이](https://github.com/La-Coruna/PS/blob/main/baekjoon/14891.cpp)

## 💡Idea
- 맞닿은 부분이 다른 극이면, 다른 방향으로의 회전이 전달된다.
- 1. 왼쪽과 오른쪽으로, 총 어디서부터 어디까지 극이 연결되는지(같이 회전하는지) 확인
- 2. 회전될 톱니바퀴들을 한번에 회전.
...
## 🗨️ Side Notes
### C++에서 음수 `%` 연산을 사용할 때 주의할 점
c++에서 음수에서 `%`(mod) 연산을 적용할 때, 음수가 나오는 것을 유의해야 한다.
 
다음 함수를 구현할 때, 주의가 필요했다.
```c++
int applyRotation(int idx){
	int res = (idx + rotationCnt) % 8;
	return (res < 0) ? res + 8 : res;
...
#### C++ vs Python 비교
Python은 항상 **0 이상**의 나머지를 반환하지만, C++에서는 **왼쪽 피연산자의 부호를 그대로 유지**한다.
- c++
```c++
cout << -3 % 8;
// 출력값: -3
