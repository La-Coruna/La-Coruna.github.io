...
- **2026-08-07**: 2026-08-07 09:31:19 (KST)경 사용 중이던 GCP 무료 평가판이 종료되어 서비스가 일시 중단된 것을 확인했습니다. 인프라 재구성 후 1~2일 내 복구할 예정입니다.
- **2026-08-08**: 서비스 복구를 완료했습니다. 현재 정상적으로 이용할 수 있습니다.
- **2026-08-12**: 이전 서비스 복구에서 비용 절감을 위해 서비스 운영 환경을 GKE Standard 기반 Spot 노드 구조로 전환했습니다. 전환 이후 Spot 노드가 교체되는 과정에서 데이터베이스보다 백엔드가 먼저 기동되며, 노드 복구 후에도 서비스 접속이 일시적으로 지연되는 문제가 발생했습니다. 이를 해결하기 위해 백엔드가 데이터베이스 연결을 일정 시간 자동으로 재시도하도록 개선했으며, 현재 서비스는 정상적으로 이용할 수 있습니다. 다만 Spot 노드 특성상 노드 교체 중에는 일시적인 접속 지연이 발생할 수 있습니다. 향후 포털은 Firebase Hosting, Cloud Run, Firestore 기반 구조로 전환하여 노드 교체와 관계없이 더 안정적으로 이용할 수 있도록 개선할 예정입니다.
